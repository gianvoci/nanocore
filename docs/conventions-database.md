...
- Falls back from MySQL to SQLite on failure. Throws if both fail.
...
- Critical: tables with non-`id` primary keys MUST pass the third argument, or `findById`/`update`/`delete` will target the wrong column.
...
| Read | `$orm->findById($id)` | Returns `self` or `null` |
...
- `findBy` and `findAll` use `(clone $this)->hydrate($row)` — each result is an independent instance.
