...
├── .env.local                # Local overrides (gitignored, overrides .env)
├── CLAUDE.md                 # Project overview and doc index
├── LICENSE                   # GPL-3.0-or-later license
├── README.md                 # Package documentation — shown on Packagist and GitHub
├── .gitignore                # Ignores .env, .env.local, .vscode, /vendor/, *.log, composer.lock, OS files
...
| `.env.local` | Local config overrides. Gitignored. Values here overwrite `.env` values. Useful for local development credentials. |
