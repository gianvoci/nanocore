...
- `curlRequest` with `'with_info' => true` returns `['body'=>mixed,'status'=>int,'content_type'=>string|null,'headers'=>array<string,string[]>]`. The `headers` key maps each response header name to an array of values (e.g. `['Content-Type' => ['application/json'], 'Set-Cookie' => ['session=abc', 'csrf=def']]`). Header names preserve original casing from the response. Without `'with_info'`, returns the body directly.
